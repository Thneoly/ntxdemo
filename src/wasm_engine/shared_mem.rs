//! Shared-memory ABI between host (this crate) and a wasm guest component.
//!
//! Goals:
//! - Host writes RX payloads + metadata (sock_id etc.) into guest linear memory.
//! - Host notifies guest via a wasm call (e.g. `notify_rx`).
//! - Guest reads descriptors and payload bytes from memory.
//!
//! This module only defines the memory layout and safe encode/decode helpers.
