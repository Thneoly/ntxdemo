...
wit_bindgen::generate!({
    world: "scheduler:actions-executor/action-executor-component@0.1.0",
...
use crate::exports::scheduler::actions_executor::action_component::Guest;
...
struct ActionExecutorComponentImpl;
...
impl ActionExecutorComponent {
    ...
    pub fn init_component(&mut self) -> Result<(), String> {
        if !self.initialized {
            ActionExecutorComponentImpl::init_component()?;
            self.initialized = true;
        }
        Ok(())
    }

    pub fn execute_action(&mut self, action: ActionDef) -> Result<ActionOutcome, String> {
        self.init_component()?;
        ActionExecutorComponentImpl::execute_action(action)
    }

    pub fn release_component(&mut self) -> Result<(), String> {
        if self.initialized {
            ActionExecutorComponentImpl::release_component()?;
            self.initialized = false;
        }
        Ok(())
    }
...
impl Default for ActionExecutorComponent {
    fn default() -> Self {
        Self::new()
    }
}
...
fn execute_http_request(
    ...
    if let Some(ip) = bind_ip {
        match socket.bind_ip(ip, 0) {
            Ok(()) => {
                println!("[ActionExecutor] Bound source IP {}", ip);
            }
            Err(err) => {
                println!(
                    "[ActionExecutor] WARN: failed to bind {} (error={:?}). Continuing without explicit source IP.",
                    ip, err
                );
            }
        }
...
impl exports::scheduler::actions_executor::action_component::Guest for ActionExecutorComponentImpl {
    fn init_component() -> Result<(), String> {
        Ok(())
    }

    fn execute_action(action: ActionDef) -> Result<ActionOutcome, String> {
        println!("[ActionExecutor] Received action: {:?}", action);
        ...
        if let Err(err) = scheduler_events::emit_for_action(
            ...
            println!(
                "[ActionExecutor] WARN: failed to emit scheduler events for `{}`: {}",
                action.id, err
            );
        }
...
export!(ActionExecutorComponentImpl);
